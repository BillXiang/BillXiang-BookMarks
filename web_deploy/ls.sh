    ...
    do
        ...
        then
            ...
            if [[ "$html" -ne 0 ]];then
                ...
                do
                    echo $a,$b,$c
                    tags=$(echo $file|awk -F'[/]' '{for (i=1;i<NF;i++) {if ($i=="contribute") {$i="Contribute"}; if ($i=="书签工具栏"||$i=="study"||$i==".") {$i=""} else {printf "<a herf=./tags/"$i">"$i"</a>";if(i!=NF-1){printf " "}else{printf "\n"}}}}')
                    cd tags
                ...
                tags=$(echo $file|awk -F'[/]' '{for (i=1;i<NF;i++) {if ($i=="书签工具栏"||$i=="study"||$i==".") {$i=""} else {printf "<a herf=./tags/"$i">"$i"</a>";if(i!=NF-1){printf " "}else{printf "\n"}}}}')
