...
wc -l tags/*|sort -r|tail -n +2|head -n 10|awk -F'[/.[:space:]]' '{print "<h5><a href=./tags/"$(NF-1)".html>"$(NF-1),"</a>"$(NF-3)"篇</h5><br>"}' | tee -a all.html index.html docs.html
wc -l tags/*|sort -r|tail -n +2|awk -F'[/.[:space:]]' '{print "<h5><a href=./tags/"$(NF-1)".html>"$(NF-1),"</a>"$(NF-3)"篇</h5><br>"}' >> all_tags.html
