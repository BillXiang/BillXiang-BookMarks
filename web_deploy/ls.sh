...
split_text() {
    ...
    if ($text =~ /([\x{4e00}-\x{9fff}])/) {
        $pos = $-[0];  # 获取第一个中文字符位置
        $english = substr($text, 0, $pos);
        $chinese = substr($text, $pos);
        # 使用不可见分隔符连接结果
        print $english . "\x1F" . $chinese;
    } else {
        print $text . "\x1F";  # 无中文时中文部分为空
    }
    '
}
    ...
    do
        ...
        then
            ...
            if [[ "$html" -ne 0 ]];then
                ...
                result=$(echo "$summary" | split_text)

                # 使用单元分隔符(0x1F)拆分结果
                IFS=$'\x1F' read -d '' -r summary_eng summary_chn <<< "$result"
                ...
                do
                    ...
                    <tr><td>$summary_eng</td></tr> \
                    <tr><td>$summary_chn</td></tr> \
