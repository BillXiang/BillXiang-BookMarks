...
sort -rk1 > sort.tmp
cat sort.tmp
cat sort.tmp |awk -F'<tr><td>' '{$1="";file[NR]=$2;$2=""}{FILENAME=file[NR];cmd = "head -n 3 " FILENAME " | tail -n 1 |cut -c6-> head.tmp" NR;result=system(cmd);getline line < ("head.tmp" NR);print "<tr><td><a href="line">原文</a></td><td>"file[NR]}{cmd = "cat head.tmp" NR;system(cmd);}' >> index.html
rm -f sort.tmp
